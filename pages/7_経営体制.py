...
st.set_page_config(page_title="🏗️ 経営体制", page_icon="🏗️", layout="wide")
style.inject()
...
PIPELINES_DEF = [
    ...
    {"name": "user_acquisition_funnel","cat": "監視",     "schedule": "無効化中 (collect_real_metrics未実装)", "agents": "monitoring_agent",         "desc": "note→製品ページ→購入のファネルメトリクスを日次更新。collect_real_metrics()実装まで停止中"},
...
with tab1:
    style.section_card_start(f"⚙️ パイプライン一覧（{n_pl} 本）")
...
with tab2:
    style.section_card_start(f"👥 エージェント一覧（{n_ag} 種）")
    departments = sorted(set(a["dept"] for a in AGENTS_DEF))
    for dept in departments:
        agents = [a for a in AGENTS_DEF if a["dept"] == dept]
        style.section_title(f"🏢 {dept}")
        rows = [{"エージェント名": a["name"], "役割": a["purpose"], "使用パイプライン": a["pipelines"]} for a in agents]
        import pandas as pd
